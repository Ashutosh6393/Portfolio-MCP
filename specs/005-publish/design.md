...
| T-41 | ~~An existing branch refuses cleanly~~ **Superseded by T-44 as of slice 4** — see *Edge cases and failure modes*: "a second publish is a clean refusal (T-41), never a silent overwrite" was true only *"until slice 4 ships."* Slice 4 makes a leftover branch the expected case (retry, or a second publish) and handles it rather than refusing. `src/services/publish.test.ts` retitles T-41 to pin what survives: a branch with no tracked pull request proceeds, writes fresh (no stale `sha`), and opens a PR — no crash, no silent overwrite. | unit | the branch already exists, no PR found for it → publish proceeds, writes without a `sha`, opens a fresh PR |
