...
- **Status:** complete — all four slices, M-1/M-2/M-3 all passed
- **Branch:** `feat/publish`
...
| — | **M-1 — a real PR on `portfolio` from a real client** | 14 | M-1 | 3 | `done` 2026-08-03 | — | PR #1, closed unmerged |
...
| — | **M-3 — publishing twice from a phone leaves one PR** | 18 | M-3 | 4 | `done` 2026-08-03 | — | same PR #1, said "updated" |
...
| 3 | M-2, Tasks 10–14, M-1 — the PR path | 5 | `complete` | — |
| 4 | Tasks 15–18, M-3 — idempotency and `revise` | 2 | `complete` | — |
...
| 2026-08-03 | `src/services/publish.test.ts` — added `T-65` (pure addition, nothing existing touched) | Closed a coverage gap found during the live M-1 run: `publish` was fixed (commit `3518ddd`, `src/lib/github.ts` and `src/services/publish.ts`) to name a missing token permission by branching on the new `GithubForbiddenError`, but the fix shipped with no test. Added `T-65`: `createPullRequest` rejects with `GithubForbiddenError("portfolio", "pulls")` → `publish` refuses naming both `Contents` and `Pull requests`, and the message contains neither "unreachable" nor "try again" — the wording a retryable, reachable-but-failed refusal would use. Asserts by `instanceof`, not by matching the fixture's Error message, matching the pattern the rest of the file already uses for `GithubNotFoundError`/`GithubAlreadyExistsError`/`GithubConflictError`. Added to `design.md` → Test cases → Slice 3, marked added after the live M-1 run. Passed on first run — the fix already existed; this closes the gap, it is not TDD-red-then-green. | A 403 is a token-scope problem, not a transient one — a retry can never fix it, and the pre-fix generic message ("GitHub did not complete the request") read exactly like one, sending the reader to check the network instead of the token's permissions. |
...
### 2026-08-03 — M-1 and M-3 passed, live

Run against the real `Portfolio-new` through the MCP handler, not a fake.

- **Health, live:** `{"site":"ok","github":"ok","schema":"ok"}` — the `schema` check had
  only ever run against a fake until now.
- **M-1:** first attempt failed **403** — the token had `Contents: write` but not
  `Pull requests: write`. Separate fine-grained permissions; ADR-005 decision 8 says
  otherwise and is wrong. Recorded in `design.md` → Facts checked during M-1.
  After the permission was added: PR #1 opened, title `Publish project: m1-scratch`,
  head `publish/project/m1-scratch` (singular), file `content/projects/m1-scratch.mdx`
  (**plural** — the T-29 hazard, confirmed correct against the real repo), body identical
  to the `design.md` template, `main` untouched.
- **The 403 also proved the half-written recovery path** (T-62/T-63) on its first real
  outing: branch created, commit landed, PR failed, error pointed at the GitHub branch URL.
  The retry then exercised the branch-exists-no-PR path for real.
- **M-3:** edited the draft through `get_content` → `save_draft` with its sha, published
  again. Result said **"updated"**, returned the **same PR #1**, and GitHub showed
  **exactly one** pull request with three commits on it. Nothing on `main` at any point.
- **Cleanup:** PR #1 closed unmerged, branch deleted, draft discarded. `main` clean.
